 ...
 */
public abstract class EntityListPanel<E extends Persistence<PK>, PK extends Serializable> extends GenericPanel {
	 ...
	 */
	private static final Logger log = Logger.getLogger(EntityListPanel.class);
	...
	public EntityListPanel() {
		...
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel create");
	...
	private void initializeComponents() {
		this.entityEditDialog = new EntityEditDialog<E, PK>();
		this.entityEditDialog.setParent(this);
	...
	private void initializeVisualComponents() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel init");
		...
		this.scrollPane.setBounds(10, 10, EntityListPanel.tableWidth, EntityListPanel.tableHeight);
		...
		this.table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					EntityListPanel.this.modifyEntity();
				}
			}
		});
		...
		this.table.setBounds(new Rectangle(0, 0, EntityListPanel.tableWidth, EntityListPanel.tableHeight));
		...
		this.searchField.addFocusListener(new FocusAdapter() {
			...
			public void focusGained(FocusEvent e) {
				EntityListPanel.this.searchField.selectAll();
			}
		});
		this.searchField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				EntityListPanel.this.filterRows();
			}
		});
		...
		this.newButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				EntityListPanel.this.newEntity();
			}
		});
		...
		this.deleteButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				EntityListPanel.this.deleteEntity();
			}
		});
		...
		this.closeButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				EntityListPanel.this.closeWindow();
			}
		});
	...
	private void setTableModel() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setTableModel");
	...
	public void setTableSize(Integer tableWidth, Integer tableHeight) {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setTableSize");

		// Verificamos el ancho de la tabla.
		if (tableWidth != null && tableWidth >= 400) {
			EntityListPanel.tableWidth = tableWidth;
		} else {
			EntityListPanel.tableWidth = 400;
		}

		// Verificamos la latura de la tabla.
		if (tableHeight != null && tableHeight >= 200) {
			EntityListPanel.tableHeight = tableHeight;
		} else {
			EntityListPanel.tableHeight = 200;
		}
	...
	public void setButtonSize(Integer buttonWidth, Integer buttonHeight) {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setButtonSize");

		// Verificamos el ancho de la tabla.
		if (buttonWidth != null && buttonWidth >= 100) {
			EntityListPanel.buttonWidth = buttonWidth;
		} else {
			EntityListPanel.buttonWidth = 100;
		}

		// Verificamos la latura de la tabla.
		if (buttonHeight != null && buttonHeight >= 40) {
			EntityListPanel.buttonHeight = buttonHeight;
		} else {
			EntityListPanel.buttonHeight = 40;
		}
	...
	private void setSize() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setSize");

		// Creamos el rectangulo principal de la ventana.
		Rectangle panel = new Rectangle(0, 0, 10 + EntityListPanel.tableWidth + 10 + EntityListPanel.buttonWidth + 10, 10
				+ EntityListPanel.tableHeight + 10 + 25 + 10);

		this.setPreferredSize(new Dimension(panel.width, panel.height));

		this.scrollPane.setBounds(10, 10, EntityListPanel.tableWidth, EntityListPanel.tableHeight);
		this.table.setBounds(0, 0, EntityListPanel.tableWidth, EntityListPanel.tableHeight);
		this.searchField.setBounds(10, 10 + EntityListPanel.tableHeight + 10, EntityListPanel.tableWidth, 25);

		this.newButton.setBounds(10 + EntityListPanel.tableWidth + 10, 10, EntityListPanel.buttonWidth, EntityListPanel.buttonHeight);
		this.modifyButton.setBounds(10 + EntityListPanel.tableWidth + 10, 10 + EntityListPanel.buttonHeight + 10, EntityListPanel.buttonWidth,
				EntityListPanel.buttonHeight);
		this.deleteButton.setBounds(10 + EntityListPanel.tableWidth + 10, 10 + EntityListPanel.buttonHeight + 10 + EntityListPanel.buttonHeight + 10,
				EntityListPanel.buttonWidth, EntityListPanel.buttonHeight);
		this.closeButton.setBounds(10 + EntityListPanel.tableWidth + 10, panel.height - 10 - EntityListPanel.buttonHeight,
				EntityListPanel.buttonWidth, EntityListPanel.buttonHeight);

		this.progressLabel.setBounds(10 + EntityListPanel.tableWidth + 10, panel.height - 10 - EntityListPanel.buttonHeight - 10
				- EntityListPanel.buttonHeight, EntityListPanel.buttonWidth, EntityListPanel.buttonHeight);
	}
	...
	public void fillList() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel fillList");
		...
		Thread thread = new Thread() {
			...
			public void run() {
				try {
					...
					EntityListPanel.this.beforeLoadEntityList();

					// Antes de vaciar, quitamos el ordenador de la tabla.
					EntityListPanel.this.tableSorter.setRowFilter(null);

					// Vaciamos el contenido de la tabla.
					EntityListPanel.this.tableModel.getDataVector().clear();

					// Leemos los datos desde la base de datos y la cargamos dentro de la tabla.
					if (EntityListPanel.this.entityService != null) {
						EntityListPanel.this.entityList = EntityListPanel.this.entityService.findAll();
					} else {
						EntityListPanel.this.entityList = new ArrayList<E>();
					}

					// Cargamos el listado dentro de la tabla.
					EntityListPanel.this.loadListToTable();

					// Volvemos a setear el filtro de las filas.
					EntityListPanel.this.tableSorter.setRowFilter(EntityListPanel.this.tableRowFilter);

				} catch (Exception e) {
					e.printStackTrace();
					// Entrada al LOG.
					EntityListPanel.log.error("EntityListPanel initList failed", e);
					...
					EntityListPanel.this.afterLoadEntityList();
				}
			}
		};
		thread.start();
	...
	private void filterRows() {
		this.tableRowFilter = new RowFilter<Object, Integer>() {
			...
			public boolean include(Entry<? extends Object, ? extends Integer> entry) {
				...
				if (EntityListPanel.this.columnsFilter != null) {

					for (int i = 0; i < entry.getValueCount(); i++) {
						// Solo para las columnas que queremos filtrar.
						if (EntityListPanel.this.filterColumn(i)) {
							if (entry.getStringValue(i).startsWith(EntityListPanel.this.searchField.getText())) {
								return true;
							}
						}
					}
	 ...
	 * 
	 * @see EntityListPanel#beforeLoadEntityList()
	 */
	...
	private void newEntity() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel newEntity");

		// Abrimos una ventana de edición para crear una entidad nueva.
		if (this.entityEditDialog != null) {
			this.entityEditDialog = this.entityEditDialog.getNewEntityDialog(this.getNewEntity());
			this.entityEditDialog.setVisible(true);

			// Una vez que terminamos con la creación de la entidad y se acepto, volvemos a cargar todas desde la base de datos.
			if (this.entityEditDialog.getCloseState() == WindowCloseState.COMMIT) {
				// Entrada al LOG.
				EntityListPanel.log.info("CREATE -> " + this.entityEditDialog.getEntityEditPanel().getEntity().toString());

				this.fillList();
			}
		} else {
			// Entrada al LOG.
			EntityListPanel.log.info("Entity create window don't define");
		}
	}
	...
	private void modifyEntity() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel modifyEntity");
		...
		if (this.table.getSelectedRow() != -1) {
			...
			if (entity != null) {
				// Entrada al LOG.
				EntityListPanel.log.info("MODIFY -> " + entity.toString());
			}

			// Abrimos una ventana de edición para modificar la entidad.
			if (this.entityEditDialog != null) {
				this.entityEditDialog = this.entityEditDialog.getEditEntityDialog(entity);
				this.entityEditDialog.setVisible(true);
				...
				// Entrada al LOG.
				EntityListPanel.log.info("Entity edit window don't define");
			}
		} else {
	...
	private void deleteEntity() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel deleteEntity");
		...
		if (this.table.getSelectedRow() != -1) {
			...
			if (JOptionPane.showConfirmDialog(this, "Desea borrar el elemento \"" + entity.toString() + "\"?", "Confirmación",
				...
				// Entrada al LOG.
				EntityListPanel.log.trace("DELETE -> " + entity.toString());
	...
	private void closeWindow() {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel closeWindow");
	...
	public void setContainerDialog(EntityListDialog<E, PK> entityListDialog) {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setContainerDialog");
		...
		if (entityListDialog != null) {
			...
			EntityListPanel.log.info("EntityListPanel setContainerDialog entityListDialog null");
		}
	}
	...
	public void setEntityService(GenericService<E, PK> entityService) {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setEntityService");

		if (entityService != null) {
			this.entityService = entityService;
		} else {
			EntityListPanel.log.info("EntityListPanel setEntityService entityService null");
		}
	}
	...
	public void setEntityEditPanel(EntityEditPanel<E, PK> entityEditPanel) {
		// Entrada al LOG.
		EntityListPanel.log.trace("EntityListPanel setEntityEditPanel");

		if (entityEditPanel != null) {
			if (this.entityEditDialog != null) {
				this.entityEditDialog.setEntityEditPanel(entityEditPanel);
			} else {
				EntityListPanel.log.info("EntityListPanel setEntityEditPanel entityEditDialog null");
			}
		} else {
			EntityListPanel.log.info("EntityListPanel setEntityEditPanel entityEditPanel null");
		}
	}
